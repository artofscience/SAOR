...
def test_taylor1(n):
    ...
    assert taylor1.dg(prob.x) == pytest.approx(prob.dg(prob.x), rel=1e-4)
    assert taylor1.ddg(prob.x) == pytest.approx(0, abs=1e-4)
...
def test_taylor2(n):
    ...
    assert taylor2.dg(prob.x) == pytest.approx(prob.dg(prob.x), rel=1e-4)
    assert taylor2.ddg(prob.x) == pytest.approx(prob.ddg(prob.x), rel=1e-4)
