...
def test_taylor1(n, h):
    ...
    taylor1.update(prob.x0, g, dg)

    # Check validity of Taylor1 expansion at expansion point X^(k)
    assert taylor1.g(prob.x0) == pytest.approx(g, rel=1e-4)
    assert taylor1.dg(prob.x0) == pytest.approx(dg, rel=1e-4)
    assert taylor1.ddg(prob.x0) == pytest.approx(0, abs=1e-4)
...
def test_taylor2(n, h):
    ...
    taylor2.update(prob.x0, g, dg, ddg)

    # Check validity of Taylor2 expansion at expansion point X^(k)
    assert taylor2.g(prob.x0) == pytest.approx(g, rel=1e-4)
    assert taylor2.dg(prob.x0) == pytest.approx(dg, rel=1e-4)
    assert taylor2.ddg(prob.x0) == pytest.approx(ddg, rel=1e-4)
...
def test_taylor1_intervening(n, h):
    ...
    taylor1 = Taylor1(ConLin())
    taylor1.update(prob.x0, g, dg)

    # Check validity of Taylor expansion at expansion point X^(k)
    assert taylor1.g(prob.x0) == pytest.approx(g, rel=1e-4)
    assert taylor1.dg(prob.x0) == pytest.approx(dg, rel=1e-4)
    # assert taylor1.ddg(prob.x0) == pytest.approx(ddg, abs=1e-4)

    # TODO: Test perturbation and correct second derivative
