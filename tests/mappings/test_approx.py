...
class TestLinearApproximation:
    ...
    def test_la(self):
        ...
        lin_approx = LA()
        lin_approx.update(x0, f, df)

        # Check validity at x0
        assert lin_approx.g(x0) + f == pytest.approx(f, self.tol)
        assert lin_approx.dg(x0) == pytest.approx(df, self.tol)
        assert lin_approx.ddg(x0) == pytest.approx(0, self.tol)
