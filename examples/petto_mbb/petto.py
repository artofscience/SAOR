...
#
from examples.petto_mbb.petsctopopt import PETScTopOpt
...
#       print('physic', end - start)
        ...
        baw1=(np.count_nonzero( x < 1e-3)+np.count_nonzero( x > 1e0 - 1e-3))/len(x)
        baw2=(np.count_nonzero(x<1e-3)+np.count_nonzero(x>1.-1e-3))/(np.count_nonzero(x >= 1e-3))
        change=np.linalg.norm(x-x_old)

        print("{}: {:.6e} {:.6e} {:.3e} {:.3e} {:.3e}".\
            format(converged.iteration - 1, f[0], infeasibility, baw1, baw2, change))
#
