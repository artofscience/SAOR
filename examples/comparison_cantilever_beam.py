...
def original():
    ...
    mma = MMA(updaterule=Svanberg1987_t(t=1/4))
    subproblem = Subproblem(Taylor1(mma), limits=[bounds])
    optimizer(problem, subproblem, IterationCount(50))
...
def mma2():
    ...
    bounds = Bounds(xmin=problem.x_min, xmax=problem.x_max)
    mma = MMA(updaterule=Svanberg1987_t(t=1/8))
...
def mixed_lp_mma():
    ...
    mma = MMA(updaterule=Svanberg2002(x_min=1, x_max=10))
...
def optimizer(problem, subproblem, converged):
    ...
    while not converged:
        ...
        x[:] = pdip(subproblem, variables=Pdipxyz)[0]
    print("\n")
