...
from sao.solvers import SolverIP_Svanberg as SvanbergIP
...
def example_dynamic_compliance(nelx=100, nely=50, volfrac=0.4, penal=3, rmin=3):
    ...
    # Instantiate a non-mixed approximation scheme
    subprob = Subproblem(approximation=Taylor1(intervening=MMA(prob.x_min, prob.x_max)),
...
def example_mechanism(nelx=100, nely=50, volfrac=0.3, penal=3, rmin=3, kin=0.01, kout=0.01):
    ...
    # Instantiate a non-mixed approximation scheme
    subprob = Subproblem(approximation=Taylor1(intervening=MMA(prob.x_min, prob.x_max)),
                         limits=[Bounds(prob.x_min, prob.x_max), MoveLimit(move_limit=0.3)])
